...
class ExpressionEvaluationRuleCompletenessTest {
    ...
    @Test
    fun `PrintScript 1_0 evaluates exactly the expressions of its own version`() {
        val rules = printScriptV1ExpressionEvaluationRules()

        for (expression in everyExpression) {
            assertEquals(
                expected = belongsToPrintScriptV1(expression),
                actual = isSupportedByAnyRule(rules, expression),
                message = "Las reglas de PrintScript 1.0 no coinciden con la version de " +
    ...
    fun `PrintScript 1_1 evaluates every expression of the language`() {
        val rules = printScriptV11ExpressionEvaluationRules(
            input = { error("El test no deberia leer entrada") },
            environmentVariables = {
                error("El test no deberia leer variables de entorno")
            },
        )
        ...
        for (expression in everyExpression) {
            assertTrue(
                actual = isSupportedByAnyRule(rules, expression),
                message = "Ninguna regla de PrintScript 1.1 evalua ${expression::class.simpleName}",
            )
        }
    }
    ...
    private fun isSupportedByAnyRule(rules: List<ExpressionEvaluationRule>, expression: Expression): Boolean {
        for (rule in rules) {
